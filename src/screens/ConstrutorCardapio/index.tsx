...
import type { Cardapio, CardapioSecao, CardapioItem, SecaoEspacador, SecaoVideo, SecaoGaleria, SecaoFaq, ImagemGaleria, ItemFaq } from '../../types/cardapioTypes'; // Certifique-se que ImagemGaleria e ItemFaq estão exportados em cardapioTypes
...
const ConstrutorCardapio: React.FC<ConstrutorCardapioProps> = ({ cardapioId, onSave, onBack }) => {
  ...
  useEffect(() => {
    if (cardapioId && cardapioId !== 'novo') {
      ...
      const dummyCardapio: Cardapio = { // Substituir por chamada API real
        id: cardapioId,
        nome: 'Cardápio de Exemplo Editável',
        ...
        conteudo: [
          { id: uuidv4(), ordem: 0, tipo: 'titulo', titulo: 'Nossas Entradas Especiais' },
          {
            id: uuidv4(), ordem: 1, tipo: 'item', titulo: 'Petiscos', items: [
              { id: uuidv4(), nome: 'Bruschetta Clássica', descricao: 'Pão italiano tostado com tomates frescos, alho, manjericão e azeite.', preco: 28.00, tags: ['Vegetariano'], alergenicos: ['Glúten'], ordem: 0 },
              { id: uuidv4(), nome: 'Dadinhos de Tapioca', descricao: 'Com queijo coalho e geleia de pimenta.', preco: 32.00, alergenicos: ['Lactose'], ordem: 1 }
            ]
          },
          ...
          { id: uuidv4(), ordem: 4, tipo: 'espacador', altura: 30, titulo: 'Espaçador Custom' },
          { id: uuidv4(), ordem: 5, tipo: 'video', videoUrl: 'https://www.youtube.com/embed/dQw4w9WgXcQ', titulo: 'Vídeo Institucional', legendaVideo: 'Nosso vídeo especial' },
          { id: uuidv4(), ordem: 6, tipo: 'galeria', titulo: 'Nossa Cozinha', imagens: [{
            id: uuidv4(), url: 'https://via.placeholder.com/300x200?text=Cozinha+1', legenda: 'Vista da Cozinha',
            ordem: 0
          }]},
          { id: uuidv4(), ordem: 7, tipo: 'faq', titulo: 'Dúvidas?', itensFaq: [{id: uuidv4(), pergunta: 'Vocês fazem reserva?', resposta: 'Sim, entre em contato.'}]}
        ],
      };
  ...
  const handleShowQrCode = (cardapioName: string) => {
    const url = `${window.location.origin}/cardapio/${cardapio.id}`; // Ajustar para a rota correta de visualização/PDF
    setQrCodeValue(url);
  ...
  const addContentElement = (type: CardapioSecao['tipo']) => {
    const newElementBase: Omit<CardapioSecao, 'tipo' | 'id' | 'ordem'> & { id?: string, ordem?: number } = { // Ajustado para permitir que id e ordem sejam definidos depois
        titulo: '', // Default
    };
    ...
    switch (type) {
      case 'titulo':
        ...
        break;
      case 'texto':
        newElement = { ...baseProps, ...newElementBase, tipo: type, texto: '<p>Clique para editar este bloco de texto...</p>', titulo: 'Nova Seção de Texto' };
        break;
      case 'item':
        ...
        return;
      case 'imagem':
        newElement = { ...baseProps, ...newElementBase, tipo: type, imagemUrl: 'https://via.placeholder.com/400x200?text=Nova+Imagem', titulo: 'Nova Imagem' };
        break;
      case 'lista':
        newElement = { ...baseProps, ...newElementBase, tipo: type, listaItems: ['Item 1', 'Item 2'], titulo: 'Nova Lista' };
        break;
      case 'divisor':
        newElement = { ...baseProps, tipo: type, titulo: 'Divisor' }; // Título para referência interna
        break;
      case 'espacador':
        newElement = { ...baseProps, tipo: type, titulo: 'Espaçador', altura: 20 } as SecaoEspacador;
        break;
      case 'video':
        newElement = { ...baseProps, tipo: type, titulo: 'Vídeo Embutido', videoUrl: '', legendaVideo: '' } as SecaoVideo;
        break;
      case 'galeria':
  ...
  const openEditElementModal = (section: CardapioSecao) => {
    ...
    switch (section.tipo) {
        case 'texto':
            currentConteudo.texto = section.texto;
            break;
        case 'imagem':
            ...
            break;
        case 'lista':
            currentConteudo.listaItems = section.listaItems?.join(';\n');
            break;
        case 'espacador':
            currentConteudo.altura = (section as SecaoEspacador).altura;
            // O título de espacador é mais uma label interna, não precisa estar no currentConteudo para edição se não for mostrado no modal.
            // Se for mostrado, section.titulo já o cobre.
            break;
        case 'video':
            ...
            break;
        case 'galeria':
            currentConteudo.imagens = (section as SecaoGaleria).imagens;
            break;
        case 'faq':
            currentConteudo.itensFaq = (section as SecaoFaq).itensFaq;
            break;
        // Tipos 'titulo' e 'item' apenas usam o 'titulo' genérico.
        // 'divisor' usa 'titulo' internamente, pode ser opcionalmente editável.
    }
  ...
  const handleSaveElementChanges = () => {
    ...
    switch (editingElement.type) {
      ...
      case 'lista':
        ...
        break;
      case 'espacador':
        (updates as Partial<SecaoEspacador>).altura = editingElement.currentConteudo.altura || 20; // Default se undefined
        break;
      case 'video':
        ...
        break;
      case 'galeria':
        (updates as Partial<SecaoGaleria>).imagens = editingElement.currentConteudo.imagens || [];
        break;
      case 'faq':
        (updates as Partial<SecaoFaq>).itensFaq = editingElement.currentConteudo.itensFaq || [];
        break;
      // Para 'item', o 'titulo' da seção de itens é a única coisa editada por este modal.
      // Para 'divisor', o 'titulo' é a única coisa (se editável), já coberto.
    }
  ...
  // --- Funções Auxiliares para Edição de Galeria no Modal ---
  const handleGalleryImageChange = (index: number, field: keyof ImagemGaleria, value: string | number) => {
      if (editingElement && editingElement.type === 'galeria' && editingElement.currentConteudo.imagens) {
          ...
          // @ts-ignore - Permitindo atribuição flexível, mas idealmente 'ordem' seria number
  ...
  // --- Funções Auxiliares para Edição de FAQ no Modal ---
  const handleFaqItemChange = (index: number, field: keyof Omit<ItemFaq, 'id' | 'ordem'>, value: string) => {
      if (editingElement && editingElement.type === 'faq' && editingElement.currentConteudo.itensFaq) {
          ...
          setEditingElement(prev => prev ? ({
              ...prev,
              currentConteudo: { ...prev.currentConteudo, itensFaq: newItensFaq }
          }) : null);
      }
  };
  ...
  const handleItemSave = () => {
    ...
    if (editingItemId) {
      ...
      if (targetSectionIdForModalItem) {
        setCardapio(prev => ({
          ...prev,
          conteudo: prev.conteudo.map(section =>
  ...
  const handleGeneratePdf = () => {
    ...
    const addWrappedText = (
        ...
        lines.forEach((line: string) => {
            ...
            if (options?.align === 'center') {
                ...
                currentX = xPos + (maxWidth - textWidth) / 2; // Centraliza em relação à maxWidth
                if (xPos === pageWidth / 2) currentX = (pageWidth - textWidth) / 2; // Centraliza na página
    ...
    if (cardapio.logoUrl) {
        try {
            const imgProps = doc.getImageProperties(cardapio.logoUrl); 
            const logoHeight = 25;
    ...
    for (const section of sortedConteudo) {
      ...
      if (section.tipo === 'titulo' && section.titulo) {
        ...
        const plainText = section.texto.replace(/<[^>]+>/g, ' '); 
        ...
        for (const item of sortedItems) {
            ...
            // Salva o y atual para posicionar o preço
            const currentYBeforeName = y;
            addWrappedText(item.nome, margin + 5, y, contentWidth - priceWidth - 10, itemFontSize, {style: 'bold'});
            
            // Calcula o y para o preço: se o nome usou uma linha, y não mudou. Se mais, y aumentou.
            // Queremos o preço na última linha do nome.
            const nameLines = doc.splitTextToSize(item.nome, contentWidth - priceWidth - 10).length;
            const priceY = currentYBeforeName + (nameLines -1) * lineHeight;
      ...
      // Adicionar renderização para 'divisor', 'espacador', 'video', 'galeria', 'faq' no PDF se desejado
      else if (section.tipo === 'divisor') {
        checkPageBreak(lineHeight * 1.5);
        doc.setLineWidth(0.5);
        doc.line(margin, y, pageWidth - margin, y);
        y += lineHeight * 1.5;
      } else if (section.tipo === 'espacador') {
        const alturaEspacador = (section as SecaoEspacador).altura || 20;
        checkPageBreak(alturaEspacador / 3); // Aproximação da altura em unidades de PDF
        y += alturaEspacador / 3;
      }
      // Para video, galeria, faq, a representação em PDF pode ser um placeholder ou um resumo
    }

    if (cardapio.informacoesAdicionais) {
        const infoHeightEstimate = descriptionFontSize * 3; 
        if (y + infoHeightEstimate > pageHeight - margin - 10) { // -10 para número da página
            doc.addPage();
            y = margin;
        }
        ...
        y = Math.max(y, footerY); // Garante que não sobreponha conteúdo anterior se houver muito espaço
    ...
    for(let i = 1; i <= pageCount; i++) {
        ...
        doc.text('Página ' + String(i) + ' de ' + String(pageCount), pageWidth - margin - 15, pageHeight - margin + 5);
    }
  ...
  return (
    <Container>
      ...
      <PdfButtonWrapper>
        ...
        <IconButton onClick={() => handleShowQrCode(cardapio.nome)} title="Gerar QR Code para Link do Cardápio">
          <FaQrcode />
        </IconButton>
      </PdfButtonWrapper>
        ...
        <Modal>
          <ModalContent>
            ...
            <FormSection style={{border: 'none', padding: 0, maxHeight: '70vh', overflowY: 'auto' }}>
                {/* Campo de Título da Seção (condicional) */}
                    ...
                    <>
                        ...
                        {editingElement.currentConteudo.imagens?.map((img, index) => (
                            <div key={img.id} style={{ border: '1px solid #ddd', padding: '15px', marginBottom: '15px', borderRadius: '4px' }}>
                                ...
                                <input
                                    type="text"
                                    id={`galleryImageUrl_${img.id}`}
                                    placeholder="https://exemplo.com/imagem.jpg"
                                    value={img.url}
                                    onChange={(e) => handleGalleryImageChange(index, 'url', e.target.value)}
                                    style={{marginBottom: '5px'}}
                                />
                                <label htmlFor={`galleryImageLegenda_${img.id}`}>Legenda da Imagem {index + 1} (opcional):</label>
                                <input
                                    type="text"
                                    id={`galleryImageLegenda_${img.id}`}
                                    value={img.legenda || ''}
